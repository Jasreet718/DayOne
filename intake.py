#Header
print(f"{'CASE':<26}{'AGE':<8}{'YEARS UNSOLVED'}")  # print column titles, left-aligned in fixed-width fields
print()  # blank line separating the header from the case rows


record1 = "CARTER, DEMARCUS | m | 36 | 2018-03-22 | 412 larkmoor lane | beat 352 | OPEN"  # raw pipe-delimited case record

#Step 1
fields = record1.strip().split(" | ")  # remove leading/trailing whitespace, then split into a list on " | "
...
record2 = "reyes, miguel|M|32|2018-05-17| 3308 DELAFORD STREET |Beat 347|open"  # raw pipe-delimited case record

#Step 1
fields = record2.strip().split("|")  # remove leading/trailing whitespace, then split into a list on "|"
...
date = fields[3]  # field 3: date the case was opened, kept as a string
year = int(date[0:4])  # first 4 characters of the date string, converted to int
addr = fields[4].strip().title()  # field 4: address, trimmed of extra spaces and converted to Title Case
status = fields[6].strip().upper()  # field 6: case status, trimmed and converted to uppercase
